...
type ActivityAction =
  ...
  | { type: 'HYDRATE'; state: ActivityState };
...
export function ActivityProvider({ children }: { children: ReactNode }) {
  const [state, dispatch] = useReducer(activityReducer, DEFAULT_STATE);
  const [ready, setReady] = useState(false);
  const isFirstRender = useRef(true);

  useEffect(() => {
    loadFromStorage<ActivityState>(STORAGE_KEY).then((saved) => {
  ...
  return (
    <ActivityContext.Provider value={{ state, dispatch, activeActivity, ready }}>
      {children}
