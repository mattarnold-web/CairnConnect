...
export function HazardReportModal({ trailName, onClose, onSubmit }: HazardReportModalProps) {
  ...
  if (submitted) {
    return (
      <div className="fixed inset-0 z-50 flex items-center justify-center bg-black/60 backdrop-blur-sm p-4">
        <div className="rounded-2xl border border-gray-200 bg-white p-8 text-center max-w-sm w-full">
  ...
  return (
    <div className="fixed inset-0 z-50 flex items-end sm:items-center justify-center bg-black/60 backdrop-blur-sm p-4">
      <div className="rounded-2xl border border-gray-200 bg-white w-full max-w-lg max-h-[90vh] overflow-y-auto">
        {/* Header */}
        <div className="flex items-center justify-between p-5 border-b border-gray-200">
          ...
          <button
            onClick={onClose}
            className="h-8 w-8 rounded-lg flex items-center justify-center text-gray-500 hover:text-gray-800 hover:bg-gray-100 transition-colors"
          >
            <X className="h-5 w-5" />
          </button>
        </div>
        ...
        <div className="p-5 space-y-6">
          <p className="text-sm text-gray-500">
            Reporting for <span className="font-medium text-gray-800">{trailName}</span>
          </p>

          {/* Hazard type selection */}
          <div>
            <label className="block text-sm font-medium text-gray-700 mb-3">What did you encounter?</label>
          ...
          <div>
            ...
            <textarea
              ...
              rows={3}
              className="w-full rounded-xl bg-gray-100 border border-gray-200 px-4 py-3 text-sm text-gray-900 placeholder:text-gray-300 focus:border-canopy focus:outline-none resize-none"
            />
          </div>

          {/* Photo upload placeholder */}
          <button className="w-full flex items-center justify-center gap-2 rounded-xl border border-dashed border-gray-200 py-3 text-sm text-gray-500 hover:text-canopy hover:border-canopy/40 transition-colors">
