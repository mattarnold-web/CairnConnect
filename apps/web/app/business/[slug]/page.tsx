...
import { MOCK_BUSINESSES, MOCK_TRAILS, MOCK_REVIEWS } from '@/lib/mock-data';
...
export default function BusinessDetailPage() {
      ...
      <div className="min-h-screen bg-cairn-bg">
        ...
        <div className="flex flex-col items-center justify-center gap-4 pt-32 text-center">
          ...
          <p className="text-slate-400">
            We could not find a business matching that URL.
          </p>
          <Link
            href="/explore"
  ...
  const todayHours = business.hours ? (business.hours as Record<string, string>)[getTodayKey()] ?? null : null;

  const reviews = MOCK_REVIEWS.filter(
    (r) => r.entity_type === 'business' && r.entity_id === business.id,
  );
  const nearbyTrails = MOCK_TRAILS.slice(0, 3);
  ...
  return (
    <div className="min-h-screen bg-cairn-bg pb-24">
      ...
      <div className="relative h-[300px] bg-gradient-to-br from-cairn-elevated via-cairn-card to-cairn-bg">
        ...
        {/* Save / Spotlight badges top right */}
          ...
          <section className="mt-8">
            ...
            <div className="flex flex-wrap gap-2">
              {business.tags.map((tag: string) => (
                <span
                  key={tag}
                  className="rounded-full bg-cairn-elevated/50 border border-cairn-border px-3 py-1 text-xs text-slate-400"
                >
                  {tag}
                </span>
              ))}
