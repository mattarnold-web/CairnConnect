...
import { DynamicMap, type MapMarker } from '@/components/map';
import { useFormat } from '@/lib/use-format';
import { submitHazardReport } from '@/lib/actions/trail-conditions';
import { toggleSavedItem } from '@/lib/actions/saved-items';
import type { DbTrail, DbBusiness } from '@/lib/database-types';
...
interface TrailDetailClientProps {
  ...
  nearbyBusinesses: DbBusiness[];
}

export function TrailDetailClient({ trail, reviews, nearbyBusinesses }: TrailDetailClientProps) {
  const fmt = useFormat();
                ...
                markers={[
                  {
                    ...
                    lng: trail.lng,
                    label: trail.name,
                    sublabel: `${DIFFICULTY_LABELS[trail.difficulty] || trail.difficulty} \u00B7 ${fmt.distance(trail.distance_meters)}`,
                    type: 'trail',
                    href: undefined,
                  },
                ]}
        ...
        <HazardReportModal
          ...
          onSubmit={async (report) => {
            ...
            await submitHazardReport({
              ...
              condition: report.condition as 'open' | 'caution' | 'closed',
